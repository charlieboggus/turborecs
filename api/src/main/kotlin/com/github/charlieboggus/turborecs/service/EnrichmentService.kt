...
class EnrichmentService(
     ...
     *
     * Important: avoid holding a DB transaction open across network calls.
     */
    ...
    private fun applyMovieEnrichment(item: MediaItemEntity, e: MovieEnrichment) {
        ...
        if (existing != null) {
            if (existing.runtimeMinutes == null) {
                existing.runtimeMinutes = e.runtimeMinutes
            }
            if (existing.genres.isEmpty() && !e.genres.isNullOrEmpty()) {
                existing.genres = e.genres as MutableList<String>
            }
    ...
    private fun applyBookEnrichment(item: MediaItemEntity, e: BookEnrichment) {
        ...
        if (existing != null) {
            if (existing.pageCount == null) {
                existing.pageCount = e.pageCount
            }
            if (existing.isbn == null) {
                existing.isbn = e.isbn
            }
